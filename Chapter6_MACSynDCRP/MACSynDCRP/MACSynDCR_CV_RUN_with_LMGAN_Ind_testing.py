...
def train_and_evaluate_model(data, labels, args, out_dir,intTestData,ind_y):
    ...
    for fold, (train_index, val_index) in enumerate(kfold.split(data)):
        ...
        ANNmodel, best_preds, train_preds,ann_ipred=Train_MACSynDCR_ANN(train_data, train_labels,val_data,val_labels, args,fold,intTestData)
        print(ann_ipred)
       ...
       # pcc, a = pearsonr([1,1,1], [1,1,4])
        ...
        kann_ipred=Train_and_Validation_MACSynDCR_ANN(train_data,val_data,train_labels,val_labels,args,intTestData)
        print("KANN",kann_ipred)
        kcnn_ipred=Train_and_Validation_MACSynDCR_CNN(train_data,val_data,train_labels,val_labels,args,intTestData)
...
def main():
    ...
    drugdata = pd.read_csv('SDDSynergyProcessDataset_0to10_2432_dti_LMGAN.csv')
...
# -------- Step 1: 80-20 Split --------
    ...
    best_model = train_and_evaluate_model(data, labels,args, out_dir,X_test,y_test) ##CV
