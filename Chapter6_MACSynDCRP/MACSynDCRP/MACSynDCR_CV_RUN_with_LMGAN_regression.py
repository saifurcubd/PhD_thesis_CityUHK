...
class TorchCNN(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Sequential(
            nn.Flatten(),
            nn.Linear(32 * 2432, 64),
            nn.ReLU(),
            nn.Linear(64, 1)
        )
    def forward(self, x): return self.fc(self.conv(x.unsqueeze(1))).squeeze()
...
def evaluate(model, X_test, y_test):
    ...
    return {
        ...
        "R^2": r2_score(y_test_np, y_pred),
        "PCC": pearsonr(y_test_np, y_pred)[0],
        "Spearman": spearmanr(y_test_np, y_pred)[0],
    }
...
def main():
    ...
    drugdata = pd.read_csv('DeepSyndergyProcessDataset_-10to20_2432_dti_LMGAN.csv')
    ...
    X_train, X_test = data[:10000], data[10000:]
    ...
    print("Torch ANN Metrics:", metrics)
